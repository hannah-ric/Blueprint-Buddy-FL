  ...
  static async search(searchQuery, currentUser, organizationId, globalAccess) {
    try {
      ...
      const tableColumns = {
        ...
        hardware_fasteners: ['name', 'description'],

        joinery_methods: ['name', 'description'],

        materials: ['name', 'description', 'mechanical_properties'],

        workshop: ['name'],
      };
      const columnsInt = {
        components: ['standard_dimensions'],

        hardware_fasteners: ['diameter', 'length'],

        joinery_methods: ['strength_rating'],

        materials: ['nominal_size', 'actual_size'],
      };
